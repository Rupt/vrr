...
main(void)
{
    ...
    test_chacha_64(
        /*label=*/1,
        /*key=*/(uint8_t[32]){[31] = 1}, /*nonce=*/(uint8_t[8]){0},
        /*expected=*/(uint8_t[64]){0x45, 0x40, 0xf0, 0x5a, 0x9f, 0x1f, 0xb2,
                                   0x96, 0xd7, 0x73, 0x6e, 0x7b, 0x20, 0x8e,
                                   0x3c, 0x96, 0xeb, 0x4f, 0xe1, 0x83, 0x46,
                                   0x88, 0xd2, 0x60, 0x4f, 0x45, 0x09, 0x52,
                                   0xed, 0x43, 0x2d, 0x41, 0xbb, 0xe2, 0xa0,
                                   0xb6, 0xea, 0x75, 0x66, 0xd2, 0xa5, 0xd1,
                                   0xe7, 0xe2, 0x0d, 0x42, 0xaf, 0x2c, 0x53,
                                   0xd7, 0x92, 0xb1, 0xc4, 0x3f, 0xea, 0x81,
                                   0x7e, 0x9a, 0xd2, 0x75, 0xae, 0x54, 0x69,
                                   0x63});
    test_chacha_64(
        /*label=*/2,
        /*key=*/(uint8_t[32]){0}, /*nonce=*/(uint8_t[8]){[7] = 1},
        /*expected=*/
        (uint8_t[64]){
    ...
    test_chacha_64(
        /*label=*/3,
        /*key=*/(uint8_t[32]){0}, /*nonce=*/(uint8_t[8]){1},
        /*expected=*/(uint8_t[64]){0xef, 0x3f, 0xdf, 0xd6, 0xc6, 0x15, 0x78,
                                   0xfb, 0xf5, 0xcf, 0x35, 0xbd, 0x3d, 0xd3,
                                   0x3b, 0x80, 0x09, 0x63, 0x16, 0x34, 0xd2,
                                   0x1e, 0x42, 0xac, 0x33, 0x96, 0x0b, 0xd1,
                                   0x38, 0xe5, 0x0d, 0x32, 0x11, 0x1e, 0x4c,
                                   0xaf, 0x23, 0x7e, 0xe5, 0x3c, 0xa8, 0xad,
                                   0x64, 0x26, 0x19, 0x4a, 0x88, 0x54, 0x5d,
                                   0xdc, 0x49, 0x7a, 0x0b, 0x46, 0x6e, 0x7d,
                                   0x6b, 0xbd, 0xb0, 0x04, 0x1b, 0x2f, 0x58,
                                   0x6b});
}
               ...
               uint8_t const expected[static const 64])
{
    ...
    for (int i = 0; i < 64; ++i) {
        if (stream[i] != expected[i]) {
            printf(vrr_observed("case %d: stream[%2d] == 0x%02x"), label, i,
                   stream[i]);
            printf(vrr_expected("case %d: stream[%2d] == 0x%02x"), label, i,
                   expected[i]);
        }
    }
}
